    ...
    public function fields(NovaRequest $request): array
    {
        ...
        return [
            ...
            Fields\Text::make(__('permissions.name'), 'name')
                ...
                ->displayUsing(fn (string $value) => Enums\Permission::tryFrom($value)?->label().'('.$value.')'),
            ...
            NovaPermission\RoleBooleanGroup::make(__('permissions.roles'), 'roles')
                ->options(fn () => (app(PermissionRegistrar::class)->getRoleClass())::all()
                    ->pluck('name', 'name')
                    ->mapWithKeys(fn (string $key, string $value) => [$key => Enums\Role::tryFrom($value)?->label() ?? $value])),
